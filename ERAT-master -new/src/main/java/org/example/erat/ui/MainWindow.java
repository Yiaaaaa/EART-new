    ...
    private void setupEventListeners() {
                    ...
                    if (!classes.isEmpty()) {
                        ...
                        if (option == JOptionPane.YES_OPTION) {
                            ...
                            confirmBtn.addActionListener(ev -> {
                                ...
                                classes.stream()
                                        .filter(c -> c.getClassId().equals(classId)) // 修正此处，添加右括号
                                        .findFirst()
        ...
        analyzeBtn.addActionListener(e -> {
            ...
            confirmBtn.addActionListener(ev -> {
                ...
                sb.append("【按学生统计】\n");
                sb.append(String.format("%-12s %-8s %-10s %-6s\n", "学号", "姓名", "班级", "缺交次数"));
                sb.append("-------------------------------------\n");
                for (Map.Entry<String, Integer> entry : byStudent.entrySet()) {
                    Student s = Student.findStudentById(entry.getKey(), students);
                    String name = (s != null) ? s.getName() : "未知";
                    String className = (s != null && s.getStuClass() != null)
                            ? s.getStuClass().getClassName()
                            : "未分配";
                    sb.append(String.format("%-12s %-8s %-10s %-6d\n",
                            entry.getKey(), name, className, entry.getValue()));
                }
            ...
            if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    if ("xlsx".equals(format)) {
                        ExportUtil.exportToExcel(currentAnalysisResult, file);
                    } else {
                        ExportUtil.exportToCsv(currentAnalysisResult, file);
                    }
